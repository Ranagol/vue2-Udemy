...
//COUNTER STUFF
import counter from './modules/counter';//import all the counter relate stuff
...
//VALUE STUFF
import * as actions from './actions';//in actions.js we could have 10 or 20 functions - although we have only one in this case. With the * we can avoid calling every function separetly. We have here all the actions for the value stuff.
import * as getters from './getters';//we have here all the getters for the value stuff
import * as mutations from './mutations';//we have here all the mutations for the value stuff
...
export const store = new Vuex.Store({//this Vuex.Store() is a special method, where we are passing an object as an argument. This Store() will handle the state of the application. Since we want to use all this on other components to, we have to export it.
  ...
  getters,//importing all the getters from getters.js
  mutations,//importing all the mutations from mutations.js
